...
if not stats or stats.Version < version then
    ...
    function stats.RevertStats(player, flag)
        ...
        elseif flag == CacheFlag.CACHE_FIREDELAY then
            ...
            if player:HasCollectible(CollectibleType.COLLECTIBLE_BRIMSTONE) and not player:HasCollectible(CollectibleType.COLLECTIBLE_HAEMOLACRIA) and not player:HasCollectible(CollectibleType.COLLECTIBLE_TECH_X) then
                if player:HasCollectible(CollectibleType.COLLECTIBLE_CHOCOLATE_MILK) then
                    tempStat = tempStat/3
                else
                    tempStat = tempStat/0.96
                end
            elseif not player:HasCollectible(CollectibleType.COLLECTIBLE_BRIMSTONE) and not player:HasCollectible(CollectibleType.COLLECTIBLE_TECH_X) then
            ...
            if player:HasCollectible(CollectibleType.COLLECTIBLE_MONSTROS_LUNG) and not player:HasCollectible(CollectibleType.COLLECTIBLE_BRIMSTONE) then
    ...
    function stats.StageBase(player, flag)
        ...
        if flag == CacheFlag.CACHE_DAMAGE then
            ...
            tempStat = tempStat*3.5
            
            tempStat = tempStat + 8 -- Polyphemus Wierd Multiplier Fix?
            
            tempStat = tempStat*mult
    ...
    function stats.StageMulti(player, cacheFlag) 
        ...
        elseif cacheFlag == CacheFlag.CACHE_FIREDELAY then
            ...
            elseif player:GetPlayerType() == PlayerType.PLAYER_AZAZEL then
                if not player:HasCollectible(CollectibleType.COLLECTIBLE_HAEMOLACRIA) and not player:HasCollectible(CollectibleType.COLLECTIBLE_TECH_X) then
                    tempStat = tempStat*3.7
                end
            end
            if player:HasCollectible(CollectibleType.COLLECTIBLE_IPECAC) then
    ...
    function stats:Save()
        saveData = stats.GenerateSave()
        stats:SaveData(saveData)
    end

    function stats:Load(fromSave)
        if fromSave then
            local saveData = stats:LoadData()
